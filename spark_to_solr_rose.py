#from cli, start with following command to open the pyspark IDE/repl
...
#create temp tables from the hive tables to query
...
#create dataframe from sql query (nothing will explicitly say that a df is being created, it's simply the default)
...
#must change data type of schedule_ship_date from string to date so that the data is properly indexed and ready to go in elasticsearch
